/* ============================================================
   site.js — client runtime for the AI Dev curriculum.
   ...
   Two things are remembered between visits, both in localStorage:
     ai-dev-theme            "light" | "dark"
     ai-dev-rail-collapsed   "1" | "0"  (desktop sidebar collapsed)
   The small script in each page's <head> reads ai-dev-theme before the
...
(() => {
  ...
  if (store.get("ai-dev-rail-collapsed") === "1") document.documentElement.classList.add("rail-collapsed");
  ...
  if (navToggle) {
    ...
    navToggle.addEventListener("click", () => {
      if (desktop.matches) {
        ...
        store.set("ai-dev-rail-collapsed", collapsed ? "1" : "0");
      } else {
  ...
  if (modal) {
    ...
    const render = (qRaw) => {
      ...
      let hits;
      if (!q) {
        hits = index.filter((x) => x.k === "lesson" || x.k === "section" || x.k === "page").slice(0, 8);
      } else {
      ...
      if (!hits.length) {
        results.innerHTML = `<div class="none">No matches for &ldquo;${esc(qRaw)}&rdquo;. Try a lesson, prompt, project step, or reference term.</div>`;
        return;
      }
      results.innerHTML = hits.map((h, i) => `
        <a class="hit" role="option" id="hit-${i}" href="${ROOT}${esc(h.u)}">
          <div class="h-name">${mark(h.t, q)}${h.s && (!h.k || !KIND_LABEL[h.k] || h.s.toLowerCase() !== KIND_LABEL[h.k].toLowerCase()) ? ` <span class="muted">&middot; ${mark(h.s, q)}</span>` : ""}${h.k && KIND_LABEL[h.k] && h.k !== "page" ? ` <span class="h-kind">${esc(KIND_LABEL[h.k])}</span>` : ""}</div>
  ...
  /* the prompt index uses the shared filter grid */
  mountFilterGrid({
    grid: "#lib-grid",
