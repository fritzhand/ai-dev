...
const COURSE = JSON.parse(readFileSync(join(ROOT, "course.config.json"), "utf8"));
const SITE_NAME = COURSE.title || CONFIG.siteName;
const TAGLINE = COURSE.tagline || CONFIG.tagline;
...
for (const root of COURSE.contentRoots || []) walk(join(ROOT, root));

const visualDirectory = join(ROOT, "web", "infographics");
const counts = {
  lessons: markdown.filter((path) => path.startsWith(join(ROOT, "lessons"))
    && !/^(index|readme)\.md$/i.test(basename(path))).length,
  prompts: markdown.reduce((total, path) =>
    total + [...readFileSync(path, "utf8").matchAll(/^\s*(?:```+|~~~+)\s*prompt\s*$/gim)].length, 0),
  routes: (COURSE.routes || []).filter((route) => route.kind === "project").length,
  visuals: existsSync(visualDirectory)
    ? readdirSync(visualDirectory).filter((file) => /\.webp$/i.test(file)).length
    : 0,
};
const signature = createHash("sha256")
  .update(JSON.stringify({ name: SITE_NAME, tagline: TAGLINE, counts }))
...
const wrap = (text, limit = 38) => {
  const sentences = String(text).match(/[^.!?]+[.!?]?/g)?.map((line) => line.trim()).filter(Boolean) || [];
  if (sentences.length > 1 && sentences.length <= 3 && sentences.every((line) => line.length <= 48)) return sentences;
  const lines = [];
  for (const word of String(text).split(/\s+/)) {
    const last = lines.at(-1);
    if (!last || (last.length + word.length + 1 > limit && lines.length < 3)) lines.push(word);
    else lines[lines.length - 1] = `${last} ${word}`;
  }
  return lines.slice(0, 3);
};
const taglineLines = wrap(TAGLINE);
const taglineSvg = taglineLines.map((line, index) =>
  `<text x="72" y="${258 + index * 64}" font-family="${FONT}" font-size="52" font-weight="700" fill="${index === taglineLines.length - 1 ? "#9fd0e2" : "#ffffff"}">${esc(line)}</text>`).join("\n  ");

/* Same course mark used by the header and generated favicon. */
const GLYPH = '<path d="M4 5h7v6H4zM13 5h7v6h-7zM4 13h7v6H4z"/><path d="m14 16 2 2 4-5"/>';
...
const chips = [
  ...
  `${counts.routes} project routes`,
  `${counts.visuals} visuals`,
].map((label) => { const c = chip(x, label); x += c.w + 14; return c.svg; }).join("\n");

const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630">
  <metadata id="build-signature">${signature}</metadata>
  <defs>
  ...
  <text x="168" y="116" font-family="${FONT}" font-size="38" font-weight="700" fill="#ffffff">${esc(SITE_NAME)}</text>
...
const chrome = (args) => {
  ...
  /* Some macOS Chrome builds finish the requested headless work but keep a
     helper alive. The timeout kills that helper; completed stdout/files remain
     valid. Any other launch failure is real. */
  if (result.error && result.error.code !== "ETIMEDOUT") throw result.error;
  if (result.status && result.status !== 0 && result.signal !== "SIGKILL") {
    throw new Error(result.stderr || `Chromium exited ${result.status}`);
  }
